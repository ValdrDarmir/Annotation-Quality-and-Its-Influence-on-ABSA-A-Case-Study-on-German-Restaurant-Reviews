...
def evaluate(model, config, results_path, lora_checkpoints, prompts_test, ground_truth_labels, label_space):
    ...
    for index, lora_path in enumerate(lora_checkpoints):
        ...
        with open(results_path_epoch + 'eval_config.txt', 'w') as f:
...
def main():
    ...
    config.model_config = '_'.join([config.task, config.dataset, config.prompt_style, str(config.learning_rate), str(config.lora_r), str(config.lora_alpha), str(config.lora_dropout), str(config.split), str(config.low_resource_setting) if config.original_split != True else 'orig'])    
